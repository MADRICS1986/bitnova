...
=======
BitNova Core version 0.16.0 is now available from:

  <https://bitnovacore.org/bin/bitnova-core-0.16.0/>
...
=======
BitNova Core 0.16.0 introduces full support for segwit in the wallet and user interfaces. A new `-addresstype` argument has been added, which supports `legacy`, `p2sh-segwit` (default), and `bech32` addresses. It controls what kind of addresses are produced by `getnewaddress`, `getaccountaddress`, and `createmultisigaddress`. A `-changetype` argument has also been added, with the same options, and by default equal to `-addresstype`, to control which kind of change is used.
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
BitNova Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
bitnova data directory. The behavior is now:
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
The minimum version of the GCC compiler required to compile BitNova Core is now 4.8. No effort will be
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- bitnova-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- bitnova-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest bitnova.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.bitnova.sprovoost.nl (Sjors)
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #11970 `b7450cd` Add test coverage for bitnova-cli multiwallet calls (ryanofsky)
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #10997 `affe927` Add option -stdinrpcpass to bitnova-cli to allow RPC password to be read from standard input (jharvell)
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #11621 `1f7695b` [build] Add temp_bitnova_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #11711 `6378e5c` bitnova_qt.m4: Minor fixes and clean-ups (fanquake)
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #11297 `16e4184` Make sure ~/.bitnova doesn't exist before build (MeshCollider)
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #11143 `3aa60b7` Fix include path for bitnova-config.h (danra)
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #10871 `c9a4aa8` Handle getinfo in bitnova-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop bitnovad (OmeGak)
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #10529 `331352f` Improve bitnovad systemd service file (Flowdalic)
>>>>>>> 5360f2baff (Initialized BitNova project)
