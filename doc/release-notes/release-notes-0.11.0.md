...
=======
BitNova Core version 0.11.0 is now available from:

  <https://bitnova.org/bin/bitnova-core-0.11.0/>
...
=======
For example, add the following to `bitnova.conf`:
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
related to the blockchain in the bitnova system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows BitNova Core to delete the raw block and undo data once 
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
block index and UTXO databases. The minimum was chosen so that BitNova Core will 
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
For an example script see [bitnova-submittx](https://github.com/laanwj/bitnova-submittx).
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #5334 `cf87536` libbitnovaconsensus: Add pkg-config support
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #6269 `95aca44` gitian: Use the new bitnova-detached-sigs git repo for OSX signatures
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #5729 `9c4a5a5` Allow unit changes for read-only BitNovaAmountField
- #5753 `0f44672` Add bitnova logo to about screen
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #5809 `46bfbe7` Add bitnova-cli man page
- #5839 `86eb461` keys: remove libsecp256k1 verification until it's actually supported
- #5749 `d734d87` Help messages correctly formatted (79 chars)
- #5884 `7077fe6` BUGFIX: Stack around the variable 'rv' was corrupted
- #5849 `41259ca` contrib/init/bitnovad.openrc: Compatibility with previous OpenRC init script variables
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
- #6116 `30dc3c1` [OSX] rename BitNova-Qt.app to BitNova-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for bitnova-tx)
>>>>>>> 5360f2baff (Initialized BitNova project)
