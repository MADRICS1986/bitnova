...
=======
The data directory is the default location where the BitNova Core files are stored.
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
Linux    | `$HOME/.bitnova/`
macOS    | `$HOME/Library/Application Support/BitNova/`
Windows  | `%LOCALAPPDATA%\BitNova\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `bitnova.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual BitNova blocks (dumped in network format, 128 MiB per file)
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
`./`               | `bitnova.conf`        | User-defined [configuration settings](bitnova-conf.md) for `bitnovad` or `bitnova-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `bitnovad.pid`        | Stores the process ID (PID) of `bitnovad` or `bitnova-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `bitnovad` or `bitnova-qt`; can be specified by `-debuglogfile` option
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [bitnova.conf](bitnova-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
`bitnova-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by BitNova Core:

Path           | Description | Repository notes
---------------|-------------|-----------------
`banlist.dat`  | Stores the addresses/subnets of banned nodes; superseded by `banlist.json` in 22.0 and completely ignored in 23.0 | [PR #20966](https://github.com/bitnova/bitnova/pull/20966), [PR #22570](https://github.com/bitnova/bitnova/pull/22570)
`blktree/`     | Blockchain index; replaced by `blocks/index/` in [0.8.0](https://github.com/bitnova/bitnova/blob/master/doc/release-notes/release-notes-0.8.0.md#improvements) | [PR #2231](https://github.com/bitnova/bitnova/pull/2231), [`8fdc94cc`](https://github.com/bitnova/bitnova/commit/8fdc94cc8f0341e96b1edb3a5b56811c0b20bd15)
`coins/`       | Unspent transaction output database; replaced by `chainstate/` in 0.8.0 | [PR #2231](https://github.com/bitnova/bitnova/pull/2231), [`8fdc94cc`](https://github.com/bitnova/bitnova/commit/8fdc94cc8f0341e96b1edb3a5b56811c0b20bd15)
`blkindex.dat` | Blockchain index BDB database; replaced by {`chainstate/`, `blocks/index/`, `blocks/revNNNNN.dat`<sup>[\[2\]](#note2)</sup>} in 0.8.0 | [PR #1677](https://github.com/bitnova/bitnova/pull/1677)
`blk000?.dat`  | Block data (custom format, 2 GiB per file); replaced by `blocks/blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> in 0.8.0 | [PR #1677](https://github.com/bitnova/bitnova/pull/1677)
`addr.dat`     | Peer IP address BDB database; replaced by `peers.dat` in [0.7.0](https://github.com/bitnova/bitnova/blob/master/doc/release-notes/release-notes-0.7.0.md) | [PR #1198](https://github.com/bitnova/bitnova/pull/1198), [`928d3a01`](https://github.com/bitnova/bitnova/commit/928d3a011cc66c7f907c4d053f674ea77dc611cc)
`onion_private_key` | Cached Tor onion service private key for `-listenonion` option. Was used for Tor v2 services; replaced by `onion_v3_private_key` in [0.21.0](https://github.com/bitnova/bitnova/blob/master/doc/release-notes/release-notes-0.21.0.md) | [PR #19954](https://github.com/bitnova/bitnova/pull/19954)
>>>>>>> 5360f2baff (Initialized BitNova project)
