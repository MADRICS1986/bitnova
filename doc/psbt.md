...
=======
# PSBT Howto for BitNova Core

Since BitNova Core 0.17, an RPC interface exists for Partially Signed BitNova
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bitnova/bips/blob/master/bip-0174.mediawiki)).
...
=======
PSBT is an interchange format for BitNova transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
...
=======
#### Multisig with multiple BitNova Core instances
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
BitNova Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitnova-cli` is used.
>>>>>>> 5360f2baff (Initialized BitNova project)
