...
=======
# Support for signing transactions outside of BitNova Core

BitNova Core can be launched with `-signer=<cmd>` where `<cmd>` is an external tool which can sign transactions and perform other functions. For example, it can be used to communicate with a hardware wallet.

## Example usage

The following example is based on the [HWI](https://github.com/bitnova-core/HWI) tool. Version 2.0 or newer is required. Although this tool is hosted under the BitNova Core GitHub organization and maintained by BitNova Core developers, it should be used with caution. It is considered experimental and has far less review than BitNova Core itself. Be particularly careful when running tools such as these on a computer with private keys on it.

When using a hardware wallet, consult the manufacturer website for (alternative) software they recommend. As long as their software conforms to the standard below, it should be able to work with BitNova Core.

Start BitNova Core:

```sh
$ bitnovad -signer=../HWI/hwi.py
...
=======
Follow the hardware manufacturers instructions for the initial device setup, as well as their instructions for creating a backup. Alternatively, for some devices, you can use the `setup`, `restore` and `backup` commands provided by [HWI](https://github.com/bitnova-core/HWI).
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
$ bitnova-cli createwallet "hww" true true "" true true true
>>>>>>> 5360f2baff (Initialized BitNova project)
```
...
=======
$ bitnova-cli -rpcwallet=<wallet> getnewaddress
$ bitnova-cli -rpcwallet=<wallet> walletdisplayaddress <address>
>>>>>>> 5360f2baff (Initialized BitNova project)
```
...
=======
Under the hood this uses a [Partially Signed BitNova Transaction](psbt.md).

```sh
$ bitnova-cli -rpcwallet=<wallet> sendtoaddress <address> <amount>
>>>>>>> 5360f2baff (Initialized BitNova project)
```
...
=======
In order to be compatible with BitNova Core any signer command should conform to the specification below. This specification is subject to change. Ideally a BIP should propose a standard so that other wallets can also make use of it.

Prerequisite knowledge:
* [Output Descriptors](descriptors.md)
* Partially Signed BitNova Transaction ([PSBT](psbt.md))
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
## How BitNova Core uses the Signer API
>>>>>>> 5360f2baff (Initialized BitNova project)
