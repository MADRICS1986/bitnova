...
=======
The BitNova Core daemon can be configured to act as a trusted "border
router", implementing the bitnova wire protocol and relay, making
...
=======
The ZeroMQ feature in BitNova Core requires the ZeroMQ API >= 4.0.0
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
    $ bitnovad -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubhashblock="tcp://[::1]:28333" \
               -zmqpubrawtx=ipc:///tmp/bitnovad.tx.raw \
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
terminator). These options can also be provided in bitnova.conf.
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
From the perspective of bitnovad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitnovad directly. Furthermore, no information is
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
using. BitNovad appends an up-counting sequence number to each
>>>>>>> 5360f2baff (Initialized BitNova project)
