...
=======
                             "RPC generatetoaddress nblocks and maxtries arguments. Example: bitnova-cli -generate 4 1000",
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to bitnovad). This changes the RPC endpoint used, e.g. http://127.0.0.1:8332/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
                "The bitnova-cli utility provides a command line interface to interact with a " CLIENT_NAME " RPC server.\n"
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
                "Usage: bitnova-cli [options] <command> [params]\n"
                "or:    bitnova-cli [options] -named <command> [name=value]...\n"
                "or:    bitnova-cli [options] help\n"
                "or:    bitnova-cli [options] help <command>\n"
>>>>>>> 5360f2baff (Initialized BitNova project)
                "\n";
...
=======
            throw std::runtime_error("-addrinfo requires bitnovad server to be running v22.0 and up");
>>>>>>> 5360f2baff (Initialized BitNova project)
        }
...
=======
                throw std::runtime_error(strprintf("invalid -netinfo level argument: %s\nFor more information, run: bitnova-cli -netinfo help", args.at(0)));
>>>>>>> 5360f2baff (Initialized BitNova project)
            }
            if (args.size() > 1) {
...
=======
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nFor more information, run: bitnova-cli -netinfo help", s));
                } else {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nThe outonly argument is only valid for a level greater than 0 (the first argument). For more information, run: bitnova-cli -netinfo help", s));
>>>>>>> 5360f2baff (Initialized BitNova project)
                }
            }
        }
...
=======
            throw std::runtime_error("-netinfo requires bitnovad server to be running v0.21.0 and up");
>>>>>>> 5360f2baff (Initialized BitNova project)
        }
...
=======
        "> bitnova-cli -netinfo\n\n"
        "The same, preceded by a peers listing without address and version columns\n"
        "> bitnova-cli -netinfo 1\n\n"
        "Full dashboard\n"
        + strprintf("> bitnova-cli -netinfo %d\n\n", NETINFO_MAX_LEVEL) +
        "Full dashboard, but with outbound peers only\n"
        + strprintf("> bitnova-cli -netinfo %d outonly\n\n", NETINFO_MAX_LEVEL) +
        "Full live dashboard, adjust --interval or --no-title as needed (Linux)\n"
        + strprintf("> watch --interval 1 --no-title bitnova-cli -netinfo %d\n\n", NETINFO_MAX_LEVEL) +
        "See this help\n"
        "> bitnova-cli -netinfo help\n"};
>>>>>>> 5360f2baff (Initialized BitNova project)
};
...
=======
                    "Make sure the bitnovad server is running and that you are connecting to the correct RPC port.\n"
                    "Use \"bitnova-cli -help\" for more info.",
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
            strPrint += " (run \"bitnova-cli -h\" for help or \"bitnova-cli listwallets\" to see which wallets are currently loaded).";
>>>>>>> 5360f2baff (Initialized BitNova project)
        }
    } else {
