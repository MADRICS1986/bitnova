...
<<<<<<< HEAD
    ...
    check_address("unix:", "unix:/var/empty/notexist/test_bitcoin.sock", "");
=======
            BOOST_CHECK_EXCEPTION(process->connect(datadir, "test_bitnova", address), std::system_error, check_notexist);
        } else {
            BOOST_CHECK_EXCEPTION(process->connect(datadir, "test_bitnova", address), std::invalid_argument, HasReason(expect_error));
        }
        BOOST_CHECK_EQUAL(address, expect_address);
    }};
    check_address("unix", "unix:/var/empty/notexist/test_bitnova.sock", "");
    check_address("unix:", "unix:/var/empty/notexist/test_bitnova.sock", "");
>>>>>>> 5360f2baff (Initialized BitNova project)
