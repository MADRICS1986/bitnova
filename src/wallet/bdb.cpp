...
=======
//! so bitnova should never create different databases with the same fileid, but
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
            errorStr = strprintf(_("%s corrupt. Try using the wallet tool bitnova-wallet to salvage or restoring a backup."), fs::quoted(fs::PathToString(file_path)));
>>>>>>> 5360f2baff (Initialized BitNova project)
            return false;
        }
    }
