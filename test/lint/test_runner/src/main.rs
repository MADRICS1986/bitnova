...
=======
            description: "Check that no symbol from bitnova-build-config.h is used without the header being included",
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
    let config_path = "./cmake/bitnova-build-config.h.in";
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
                    // These are exceptions which don't use bitnova-build-config.h, rather CMakeLists.txt adds
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
                    ...
                    "#include <bitnova-build-config.h>" // Catch redundant includes with and without the IWYU pragma
>>>>>>> 5360f2baff (Initialized BitNova project)
                },
                "--",
            ])
...
=======
One or more files use a symbol declared in the bitnova-build-config.h header. However, they are not
including the header. This is problematic, because the header may or may not be indirectly
included. If the indirect include were to be intentionally or accidentally removed, the build could
still succeed, but silently be buggy. For example, a slower fallback algorithm could be picked,
even though bitnova-build-config.h indicates that a faster feature is available and should be used.
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
None of the files use a symbol declared in the bitnova-build-config.h header. However, they are including
>>>>>>> 5360f2baff (Initialized BitNova project)
