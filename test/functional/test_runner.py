...
=======
`test/functional/test_framework/test_framework.py:BitNovaTestFramework.main`.
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
    'interface_bitnova_cli.py --legacy-wallet',
    'interface_bitnova_cli.py --descriptors',
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
    enable_bitnovad = config["components"].getboolean("ENABLE_BITCOIND")

    if not enable_bitnovad:
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
    # Warn if bitnovad is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "bitnovad"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a bitnovad process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
>>>>>>> 5360f2baff (Initialized BitNova project)
    except OSError:
        # pgrep not supported
        pass
...
=======
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the BitNova functional test suite. "
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
    # a hard link or a copy on any platform. See https://github.com/bitnova/bitnova/pull/27561.
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
    commands per `bitnova-cli help` (`rpc_interface.txt`).
>>>>>>> 5360f2baff (Initialized BitNova project)
