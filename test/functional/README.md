...
=======
  The CI linter job also checks this, but [possibly not in all cases](https://github.com/bitnova/bitnova/pull/14884#discussion_r239585126).
...
=======
- When subclassing the BitNovaTestFramework, place overrides for the
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
  methods in the `BitNovaTestFramework` class, which can be used to skip a whole test
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
with the bitnovad(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a bitnovad.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the BitNova Core node application logic. For custom behaviour, subclass the
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
The [`TestShell`](test-shell.md) class exposes the BitNovaTestFramework
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
Taken from the [python-bitnovarpc repository](https://github.com/jgarzik/python-bitnovarpc).
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
Test objects for interacting with a bitnovad node over the p2p interface.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-bitnovalib)
>>>>>>> 5360f2baff (Initialized BitNova project)
