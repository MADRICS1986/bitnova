...
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Check that it's not possible to start a second bitnovad instance using the same datadir or wallet."""
import random
import string
...
=======
class FilelockTest(BitNovaTestFramework):
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
        self.log.info("Check that we can't start a second bitnovad instance using the same datadir")
        expected_msg = f"Error: Cannot obtain a lock on directory {datadir}. {self.config['environment']['CLIENT_NAME']} is probably already running."
        self.nodes[1].assert_start_raises_init_error(extra_args=[f'-datadir={self.nodes[0].datadir_path}', '-noserver'], expected_msg=expected_msg)

        self.log.info("Check that we can't start a second bitnovad instance using the same blocksdir")
        expected_msg = f"Error: Cannot obtain a lock on directory {blocksdir}. {self.config['environment']['CLIENT_NAME']} is probably already running."
        self.nodes[1].assert_start_raises_init_error(extra_args=[f'-blocksdir={self.nodes[0].datadir_path}', '-noserver'], expected_msg=expected_msg)

        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second bitnovad using the same datadir/blocksdir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second bitnovad instance shutdown
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
                self.log.info("Check that we can't start a second bitnovad instance using the same wallet")
>>>>>>> 5360f2baff (Initialized BitNova project)
                if descriptors:
