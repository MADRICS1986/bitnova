...
=======
  # See https://github.com/bitnova/bitnova/pull/28359#issuecomment-1698694748
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
    ${CI_RETRY_EXE} git clone --depth=1 https://github.com/bitnova-core/qa-assets "${DIR_QA_ASSETS}"
>>>>>>> 5360f2baff (Initialized BitNova project)
  fi
  (
    cd "${DIR_QA_ASSETS}"
...
=======
    ${CI_RETRY_EXE} curl --location --fail https://github.com/bitnova-core/qa-assets/raw/main/unit_test_data/script_assets_test.json -o "${DIR_UNIT_TEST_DATA}/script_assets_test.json"
>>>>>>> 5360f2baff (Initialized BitNova project)
  fi
fi
...
<<<<<<< HEAD
  ...
  echo > "${HOME}/.bitcoin"
=======
  echo > "${HOME}/Library/Application Support/BitNova"
else
  echo > "${HOME}/.bitnova"
>>>>>>> 5360f2baff (Initialized BitNova project)
fi
...
=======
  DIR_UNIT_TEST_DATA="${DIR_UNIT_TEST_DATA}" LD_LIBRARY_PATH="${DEPENDS_DIR}/${HOST}/lib" "${BASE_OUTDIR}"/bin/test_bitnova --catch_system_errors=no -l test_suite
>>>>>>> 5360f2baff (Initialized BitNova project)
fi
...
=======
  cmake -B /tidy-build -DLLVM_DIR=/usr/lib/llvm-"${TIDY_LLVM_V}"/cmake -DCMAKE_BUILD_TYPE=Release -S "${BASE_ROOT_DIR}"/contrib/devtools/bitnova-tidy
  cmake --build /tidy-build "$MAKEJOBS"
  cmake --build /tidy-build --target bitnova-tidy-tests "$MAKEJOBS"
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
  if ! ( run-clang-tidy-"${TIDY_LLVM_V}" -quiet -load="/tidy-build/libbitnova-tidy.so" "${MAKEJOBS}" | tee tmp.tidy-out.txt ); then
>>>>>>> 5360f2baff (Initialized BitNova project)
...
=======
           -- -Xiwyu --cxx17ns -Xiwyu --mapping_file="${BASE_ROOT_DIR}/contrib/devtools/iwyu/bitnova.core.imp" \
>>>>>>> 5360f2baff (Initialized BitNova project)
